...
const gameData = {
  ...
  "IQ": [
    ...
    { name: "Robin's IQ", character: "Robin", power: 9, rarity: "Epic" },
  ...
  "Body": [
   {name: "IMU's Body",character:"IMU",power:10,rarity:"Legendary"},
    ...
    { name: "Zoro's Body", character: "Zoro", power: 6, rarity: "Epic" },
    { name: "Sanji's Body", character: "Sanji", power: 5, rarity: "Rare" },
    { name: "Luffy's Body", character: "Luffy", power: 5, rarity: "Rare" },
...
const ranks = [
  { name: "God Level", minPower: 46, description: "The power of Joy Boy! You've transcended mortal limits!" },
...
// Storage keys
const STORAGE_KEYS = {
  users: 'onePieceGame_users',
  currentUser: 'onePieceGame_currentUser',
  leaderboard: 'onePieceGame_leaderboard'
};
...
function loadUserData() {
  registeredUsers = loadFromStorage(STORAGE_KEYS.users, {});
}

// Load leaderboard data from localStorage
function loadLeaderboardData() {
  globalLeaderboard = loadFromStorage(STORAGE_KEYS.leaderboard, []);
}

// Check for auto-login
function checkAutoLogin() {
  const savedUser = loadFromStorage(STORAGE_KEYS.currentUser);
  if (savedUser && registeredUsers[savedUser]) {
...
function handleRegister(e) {
  ...
  if (registeredUsers[username]) {
    showMessage('Pirate name already taken! Choose a different one.', 'error');
    return;
  }
  ...
  showMessage(`Welcome aboard, Captain ${username}! You can now login!`, 'success');
...
function handleLogin(e) {
  ...
  if (!registeredUsers[username]) {
    showMessage('Pirate not found! Please register first.', 'error');
    return;
  }
  
  if (registeredUsers[username].password !== password) {
    showMessage('Wrong password! Try again, pirate!', 'error');
    return;
  }
  ...
  showMessage(`Welcome back, Captain ${username}!`, 'success');
...
function updateLeaderboard() {
  ...
  if (globalLeaderboard.length === 0) {
    leaderboardDiv.innerHTML = '<div style="text-align: center; color: #888; padding: 20px;">No legendary combinations yet!<br>Be the first to make history!</div>';
    return;
  }
  
  // Sort leaderboard by total power
  const sortedLeaderboard = [...globalLeaderboard].sort((a, b) => b.totalPower - a.totalPower);
  ...
  sortedLeaderboard.forEach((entry, index) => {
    ...
    const comboItems = entry.combo.join(', ');
    
    leaderboardItem.innerHTML = `
      <div class="combo-rank">#${index + 1} - ${entry.rank}</div>
      <div class="combo-user">${entry.user}</div>
      <div class="combo-details">
        <div>Total Power:</div>
        <div class="combo-power">${entry.totalPower}</div>
      </div>
      <div class="combo-items">${comboItems}</div>
    `;
...
function calculateFinalRank() {
  ...
  if (currentUser && registeredUsers[currentUser]) {
    const newScore = {
      totalPower,
      rank: rank.name,
      combo: Object.values(selectedChoices).map(choice => choice.name),
      date: new Date().toISOString()
    };
    
    registeredUsers[currentUser].scores.push(newScore);
    saveToStorage(STORAGE_KEYS.users, registeredUsers);
  }
  
  // Add to global leaderboard
  const comboArray = Object.values(selectedChoices).map(choice => choice.name);
  const newEntry = {
  ...
  globalLeaderboard.sort((a, b) => b.totalPower - a.totalPower);
  
  // Keep only top 50 all-time records
  if (globalLeaderboard.length > 50) {
    globalLeaderboard = globalLeaderboard.slice(0, 50);
  }
  
  // Save leaderboard
  saveToStorage(STORAGE_KEYS.leaderboard, globalLeaderboard);
